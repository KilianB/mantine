...
function CarouselCardsDemos(props) {
  const theme = useMantineTheme();
  const mobile = useMediaQuery(`(max-width: ${theme.breakpoints.sm})`);
  const slides = data.map((item) => /* @__PURE__ */ React.createElement(Carousel.Slide, {
    key: item.title
  }, /* @__PURE__ */ React.createElement(Card, __spreadValues({}, item))));
  return /* @__PURE__ */ React.createElement(Carousel, __spreadValues({
    slideSize: "50%",
    breakpoints: [{ maxWidth: "sm", slideSize: "100%", slideGap: 2 }],
    slideGap: "xl",
    align: "start",
...
export { CarouselCardsDemos, cards };
